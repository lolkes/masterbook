...
@app.get("/health")
def health():
    return {"status": "ok", "version": VERSION, "service": "MasterBook"}
...
@app.get("/api/version")
def api_version():
    return {"version": VERSION, "features": ["dashboard", "telegram-auth", "per-user-data", "job-status", "payment-status", "search", "csv-export", "json-backup", "crud"]}
