...
from database import get_connection, _fetchall, _fetchone, _execute, _insert_id
...
async def startup():
    # Tables are created by the Neon setup migration. No destructive changes are made here.
    pass


@app.get("/")
def index():
    return FileResponse(BASE / "web" / "chat" / "index.html")


@app.get("/style.css")
def css():
    return FileResponse(BASE / "web" / "chat" / "style.css", media_type="text/css")


@app.get("/app.js")
def js():
    return FileResponse(BASE / "web" / "chat" / "app.js", media_type="application/javascript")


@app.get("/health")
def health():
    return {"status": "ok", "service": "ChatGo", "version": "1.0"}
...
def register(data: Register):
    ...
    uid = _insert_id(
        "INSERT INTO chat_users(username, display_name, password_hash, last_seen) VALUES (%s,%s,%s,NOW()) RETURNING id",
        (username, data.display_name.strip(), hash_password(data.password)),
    )
    token = secrets.token_urlsafe(32)
    SESSIONS[token] = uid
    return {"token": token, "user": {"id": uid, "username": username, "display_name": data.display_name.strip()}}
...
@app.get("/api/me")
def me(token: str | None = Cookie(default=None), authorization: str | None = None):
    raw = token or (authorization.removeprefix("Bearer ") if authorization else None)
    return user_from_token(raw)


@app.get("/api/users")
def users(q: str = "", token: str | None = Cookie(default=None), authorization: str | None = None):
    raw = token or (authorization.removeprefix("Bearer ") if authorization else None)
    me_user = user_from_token(raw)
    like = f"%{q.strip()}%"
    rows = _fetchall("SELECT id, username, display_name, avatar_url, last_seen FROM chat_users WHERE id<>%s AND (username ILIKE %s OR display_name ILIKE %s) ORDER BY display_name LIMIT 50", (me_user["id"], like, like))
    return rows


@app.post("/api/conversations/{other_id}")
def conversation(other_id: int, token: str | None = Cookie(default=None), authorization: str | None = None):
    raw = token or (authorization.removeprefix("Bearer ") if authorization else None)
    me_user = user_from_token(raw)
    if other_id == me_user["id"] or not _fetchone("SELECT id FROM chat_users WHERE id=%s", (other_id,)):
...
def conversations(token: str | None = Cookie(default=None), authorization: str | None = None):
    raw = token or (authorization.removeprefix("Bearer ") if authorization else None)
    me_user = user_from_token(raw)
    return _fetchall("""SELECT c.id, u.id AS user_id, u.username, u.display_name, u.avatar_url,
        ...
        JOIN chat_conversation_members cm ON cm.conversation_id=c.id AND cm.user_id=%s
        JOIN chat_conversation_members other ON other.conversation_id=c.id AND other.user_id<>%s
        JOIN chat_users u ON u.id=other.user_id
...
def messages(cid: int, limit: int = 100, token: str | None = Cookie(default=None), authorization: str | None = None):
    raw = token or (authorization.removeprefix("Bearer ") if authorization else None)
    me_user = user_from_token(raw)
    member = _fetchone("SELECT 1 FROM chat_conversation_members WHERE conversation_id=%s AND user_id=%s", (cid, me_user["id"]))
    if not member:
        raise HTTPException(403, "Нет доступа к чату")
    return _fetchall("SELECT m.id, m.sender_id, u.display_name AS sender_name, m.body, m.created_at, m.deleted_at FROM chat_messages m JOIN chat_users u ON u.id=m.sender_id WHERE m.conversation_id=%s ORDER BY m.created_at DESC LIMIT %s", (cid, max(1, min(limit, 200))))[::-1]
...
async def websocket_chat(websocket: WebSocket, cid: int):
    ...
    if not token or token not in SESSIONS:
        ...
        return
    uid = SESSIONS[token]
    member = _fetchone("SELECT 1 FROM chat_conversation_members WHERE conversation_id=%s AND user_id=%s", (cid, uid))
    if not member:
        await websocket.close(code=1008)
        return
    await websocket.accept()
    SOCKETS.setdefault(cid, set()).add(websocket)
    touch(uid)
    try:
        while True:
        ...
        SOCKETS.get(cid, set()).discard(websocket)
        touch(uid)
